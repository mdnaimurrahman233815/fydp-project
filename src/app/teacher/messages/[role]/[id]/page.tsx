...
export default async function TeacherThreadPage({
  ...
  if (!person) notFound();
  const thread = await listThread("teacher", user.id, role, otherId);
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        ...
        actions={
          <Link className="btn btn-ghost" href="/teacher/messages">
            Back to inbox
          </Link>
        }
      />
      <div className="card space-y-3 p-5">
        {thread.length === 0 ? <p className="text-sm text-[#6b6256]">No messages yet.</p> : null}
        {thread.map((message) => {
          const mine = message.senderRole === "teacher" && message.senderId === user.id;
          return (
