...
export async function seedDatabase() {
  ...
  await db.transaction(async (tx) => {
    ...
    const teacherRows = await tx.select().from(teachers);
    ...
    const studentRows = await tx.select().from(students);
    ...
    if (!alphaRes || !betaRes || !gammaRes || !deltaRes) {
      throw new Error("Group seed failed");
    }

    const alpha = { id: alphaRes.id };
    const beta = { id: betaRes.id };
    const gamma = { id: gammaRes.id };
    const delta = { id: deltaRes.id };
